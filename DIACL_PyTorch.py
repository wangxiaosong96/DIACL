    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        with torch.no_grad():
            ...
            test_ret = eval_PyTorch(_model, data_generator)
            torch.cuda.empty_cache()
        ...
        perf_str = 'Epoch %2d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f + %.5f], test-recall=[%.4f], test-ndcg=[%.4f]' % \
                   (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, cen_loss, cl_loss, test_ret['recall'],
                    test_ret['ndcg'])
    ...
    with torch.no_grad():
        ...
        final_test_ret = eval_PyTorch(_model, data_generator)
    ...
    pref_str = 'Final Test Set Result: test-recall=[%.4f], test-ndcg=[%.4f]' % (final_test_ret['recall'], final_test_ret['ndcg'])
